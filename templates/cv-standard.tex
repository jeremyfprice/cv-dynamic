...
\noindent\emph{Graduate \vspace{0.01in}}
    ...
    \BLOCK{for item in education | selectattr('level', 'eq', 'graduate') | sort(attribute = 'year', reverse = True)}


        \ind \VAR{item.degree}, \VAR{item.institution}, \VAR{item.year}
...
\medskip
  ...
  \BLOCK{for item in grants | sort(attribute = 'title') | sort(attribute = 'start_year', reverse = True) | sort(attribute = 'end_year', reverse = True)}


      \ind \VAR{item.institution}, ``\VAR{item.title},'' \VAR{item.role}, \$\VAR{'{:,}'.format(item.amount)}, \VAR{item.start_year}\BLOCK{-if item.start_year != item.end_year-}~- \VAR{item.end_year}\BLOCK{-endif-}
